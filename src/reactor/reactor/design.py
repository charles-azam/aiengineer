...
from .systems_reactor import smr_system, reactor_core, primary_loop, secondary_loop, containment
from .simulation_reactor import (
    calculate_power_output, calculate_annual_energy_production,
    calculate_fuel_consumption, calculate_primary_loop_parameters,
    calculate_secondary_loop_parameters, run_all_simulations
)
from .tools_reactor import (
    calculate_levelized_cost, calculate_core_volume,
    calculate_core_power_density, estimate_construction_time,
...
df_params = pd.DataFrame([
    ...
    {"Parameter": "Electrical Output", "Value": f"{REACTOR_PARAMS.electrical_power}"},
    {"Parameter": "Thermal Efficiency", "Value": f"{REACTOR_PARAMS.thermal_efficiency*100:.1f}%"},
    {"Parameter": "Design Life", "Value": f"{REACTOR_PARAMS.design_life} years"},
    {"Parameter": "Refueling Interval", "Value": f"{REACTOR_PARAMS.refueling_interval} months"},
    {"Parameter": "Availability Factor", "Value": f"{REACTOR_PARAMS.availability_factor*100:.1f}%"},
    {"Parameter": "Core Dimensions", "Value": f"{REACTOR_PARAMS.core_height} × {REACTOR_PARAMS.core_diameter} (H×D)"},
    {"Parameter": "Containment Dimensions", "Value": f"{REACTOR_PARAMS.containment_height} × {REACTOR_PARAMS.containment_diameter} (H×D)"},
])
...
core_volume = calculate_core_volume(REACTOR_PARAMS.core_height, REACTOR_PARAMS.core_diameter)
...
containment_volume = calculate_containment_volume(REACTOR_PARAMS.containment_height, REACTOR_PARAMS.containment_diameter)
...
annual_energy = REACTOR_PARAMS.electrical_power.magnitude * 8760 * REACTOR_PARAMS.availability_factor
discount_rate = 0.07  # 7%

lcoe = calculate_levelized_cost(
    capital_cost, annual_om_cost, fuel_cost, decommissioning_cost,
    annual_energy, discount_rate, REACTOR_PARAMS.design_life
)
